...
# Canonical launcher for the full Pull -> Lift -> Hang AutoOps viewer.
...
LOG_PATH="${LOG_PATH:-/tmp/franka_long_viser.log}"
...
docker exec -d \
    ...
    "cd /workspace/isaaclab && exec ./isaaclab.sh -p \
      ...
      ${CAMERA_ARGS} \
      --auto-ops --auto-ops-task long --hold-after-auto-ops \
      --max-steps 0 --substeps 4 \
      >${LOG_PATH} 2>&1"

echo "Long AutoOps Viser is starting (Pull -> Lift -> Hang, Newton + Pink)."
